...
import { handleSellerAccountUpdated } from './providers/stripe'
 ...
 *     tags:
 *       - Stripe Webhook
 *     summary: Stripe webhook endpoint for payment and payout events.
 *     description: >
 *       Receives webhook events from Stripe such as `payment_method.attached`,
 *       `checkout.session.completed`, and `payout.paid`. This endpoint must receive raw payloads
 *       and be verified using Stripe's signature header.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             description: Stripe event payload (must match Stripe event schema)
 *     responses:
 *       '200':
 *         description: Successfully received and handled the Stripe event.
 *       '400':
 ...
 *                   type: string
 *       '500':
 *         description: Server error while handling Stripe event.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
webhookRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.status(400).json({ errorMessage: 'Invalid Stripe signature' })
    }

    try {
      switch (event.type) {
      case 'account.updated':
        await handleSellerAccountUpdated(event)
      ...
      res.status(500).json({ errorMessage: 'Failed to process Stripe event' })
    }
  }
)
