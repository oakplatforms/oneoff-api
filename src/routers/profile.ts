 ...
 *     tags:
 *       - Profile
 *     summary: Retrieve a profile by username (root route).
 *     description: Fetches the details of a profile by its unique username. This is the primary profile access route. You can optionally include related entities using the `include` query parameter.
 *     parameters:
 *       - in: path
 *         name: username
 *         required: true
 ...
 *       '404':
 *         description: Profile not found with the given username.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
  ...
  try {
    ...
    if (profile) {
      ...
      return res.status(404).json({
        errorMessage: 'No profile found with username: ' + username
      })
    }
  } catch (error) {
    const { statusCode, prismaError, customError } = generatePrismaError(error as Prisma.PrismaClientKnownRequestError)
    console.error('GET_PROFILE_BY_USERNAME_ROOT_ERROR:', prismaError, customError)
